...
export function AddParentPage() {
  ...
  const addParent = useCareStore(s => s.addParent);
  ...
  const share = async () => {
    const inviteMessage = `Join my CareCircle family! Scan the QR code or enter code: ${shortCode}`;
    const joinUrl = typeof window !== 'undefined' ? `${window.location.origin}/parent/scan?code=${shortCode}` : undefined;
    const res = await shareOrCopyInvite({
      title: 'Join my CareCircle',
      text: inviteMessage,
      url: joinUrl,
    });
  ...
  return (
    <ChildShell title="Add a parent" subtitle="A simple way to bring your family together.">
      <div className="max-w-xl">
        <div className="rounded-2xl border border-card-border bg-card p-6 card-shadow sm:p-8">
          {!code ? (
            <form onSubmit={generate} className="space-y-6">
              ...
              <div>
                ...
                <Select value={relationship} onValueChange={setRelationship}>
                  <SelectTrigger id="relationship" className="mt-2 h-12">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {['Mom', 'Dad', 'Grandmother', 'Grandfather', 'Other'].map(x => (
            ...
            <div className="text-center">
              ...
              <h2 className="text-xl font-bold font-display">Invite {name}</h2>
              ...
              {shortCode && (
                <div className="mx-auto my-6 max-w-sm rounded-2xl border border-primary/20 bg-primary/5 p-4 text-center">
                  <div className="text-xs font-bold uppercase tracking-wider text-primary">
                    Option 2: 6-Character Short Code
                  </div>
                  <p className="mt-1 text-xs text-muted-foreground">
                    Your parent can also manually type this code in the parent app instead of scanning:
                  </p>
                  <div className="my-3 flex items-center justify-center gap-2">
                    <span
                      ...
                      className="rounded-xl border bg-card px-4 py-2 font-mono text-2xl font-black tracking-widest text-primary shadow-xs select-all cursor-pointer hover:border-primary/50 transition-colors"
                    >
                      {shortCode}
                    </span>
                    <Button
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    Case-insensitive · Single-use for privacy
                  </p>
                </div>
              )}
              <div className="flex flex-wrap justify-center gap-3">
                <Button variant="outline" onClick={download}>
                  {downloaded ? (
                    <>
                      <Check className="size-4 mr-1 text-success" /> Downloaded!
                    </>
                  ) : (
                    <>
                ...
                <Button onClick={share}>
                  {shared ? (
                    <>
                      <Check className="size-4 mr-1 text-white" /> Invite copied!
                    </>
                  ) : (
                    <>
                      <Share2 className="size-4 mr-1" /> Share invite
                    </>
                  )}
                </Button>
              </div>
